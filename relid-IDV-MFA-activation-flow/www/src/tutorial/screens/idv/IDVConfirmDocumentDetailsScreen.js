 ...
 * 5. User clicks close button -> calls resetAuthState() to cancel IDV flow
 *
 * Usage:
...
const IDVConfirmDocumentDetailsScreen = {
  ...
  async handleClose() {
    ...
    try {
      console.log('IDVConfirmDocumentDetails - Calling resetAuthState to cancel IDV flow');
      await rdnaService.resetAuthState();
      console.log('IDVConfirmDocumentDetails - ResetAuthState successful');
    } catch (error) {
      console.error('IDVConfirmDocumentDetails - ResetAuthState error:', JSON.stringify(error, null, 2));
