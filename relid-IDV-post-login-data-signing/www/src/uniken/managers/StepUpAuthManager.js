...
const StepUpAuthManager = {
  ...
  _context: {
    ...
    userID: '',
    sessionParams: {},
    onCancel: null,  // Callback when user cancels password dialog
    onSubmit: null   // Callback when user submits password
  },
   ...
   * @param {Function} context.onCancel - Callback when user cancels password dialog
   * @param {Function} context.onSubmit - Callback when user submits password
   */
  setContext(context) {
    console.log('StepUpAuthManager - Setting context:', JSON.stringify({
      notificationUUID: context.notificationUUID,
      notificationTitle: context.notificationTitle,
      action: context.action,
      userID: context.userID,
      hasOnCancel: !!context.onCancel,
      hasOnSubmit: !!context.onSubmit
    }, null, 2));
    ...
    this._context = {
      ...
      onCancel: context.onCancel || null,
      onSubmit: context.onSubmit || null
    };
  },
  ...
  handlePasswordSubmit(password) {
    ...
    // Call onSubmit callback if provided (e.g., to keep loading indicator visible)
    if (this._context.onSubmit) {
      console.log('StepUpAuthManager - Calling onSubmit callback');
      this._context.onSubmit();
    }
  ...
  handleCancel() {
    ...
    // Call onCancel callback if provided (e.g., to hide loading indicator)
    if (this._context.onCancel) {
      console.log('StepUpAuthManager - Calling onCancel callback');
      this._context.onCancel();
    }
