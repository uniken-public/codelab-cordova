...
const GetNotificationsScreen = {
  ...
  setupEventListeners() {
    ...
    if (drawerNotificationHistoryLink) {
      drawerNotificationHistoryLink.onclick = (e) => {
        e.preventDefault();
        NavigationService.closeDrawer();
        console.log('GetNotificationsScreen - Navigating to NotificationHistory');
    ...
    if (drawerDeviceMgmtLink) {
      drawerDeviceMgmtLink.onclick = (e) => {
        e.preventDefault();
        NavigationService.closeDrawer();
        console.log('GetNotificationsScreen - Navigating to DeviceManagement');
    ...
    if (drawerLdaTogglingLink) {
      drawerLdaTogglingLink.onclick = (e) => {
        e.preventDefault();
        NavigationService.closeDrawer();
        console.log('GetNotificationsScreen - Navigating to LDAToggling');
        // Navigate to LDA Toggling with current session params
    ...
    if (drawerDataSigningLink) {
      drawerDataSigningLink.onclick = (e) => {
        e.preventDefault();
        NavigationService.closeDrawer();
        console.log('GetNotificationsScreen - Navigating to DataSigningInput');
        // Navigate to Data Signing with current session params
  ...
  registerSDKEventHandlers() {
    ...
    // Note: getPassword (challengeMode 3) is handled centrally by SDKEventProvider
    // We just need to set context before calling updateNotification
  ...
  handleActionButtonClick(actionValue) {
    ...
    if (typeof StepUpAuthManager !== 'undefined') {
      StepUpAuthManager.setContext({
        ...
        // Callback when user cancels password dialog
        onCancel: () => {
          console.log('GetNotificationsScreen - Password dialog cancelled by user');
          this.showLoading(false);  // Hide loading indicator
        },

        // Callback when user submits password
        onSubmit: () => {
          console.log('GetNotificationsScreen - Password submitted, waiting for onUpdateNotification event');
          // Keep loading indicator visible, wait for onUpdateNotification event
        }
      });
    }
  ...
  /**
   * Handle updateNotification response event
   *
   * This handler receives the event after:
   * - Password step-up authentication (challengeMode 3) completes
   * - IDV selfie step-up authentication (idvWorkflow 9) completes
   * - LDA authentication completes
   *
   * Pattern (matches AdditionalDocumentScanScreen):
   * 1. Navigate back to GetNotifications screen
   * 2. Show alert with 300ms delay
   * 3. Refresh notifications after alert
   */
  handleUpdateNotificationResponse(data) {
    ...
    const errorString = data.error?.errorString || 'Failed to update notification';

    // FIRST: Check error code (API-level error, including LDA cancelled)
    if (errorCode !== 0) {
      console.error('GetNotificationsScreen - API error:', errorCode, errorString);

      // Handle LDA cancelled (errorCode 131) - Silent return
      if (errorCode === 131) {
        console.log('GetNotificationsScreen - LDA cancelled by user, silent return');
        // Navigate back to GetNotifications without showing error
    ...
    // SECOND: Check status code (backend validation)
    if (statusCode === 100) {
      // Success case
      ...
      setTimeout(() => {
        alert(statusMsg || 'Notification action completed successfully');
        this.loadNotifications(); // Refresh after alert
      }, 300);
