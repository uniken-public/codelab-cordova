  ...
  async handleClose() {
    ...
    try {
      if (this.state.challengeMode === 8) {
        ...
        console.log('IDVSelfieConfirmation - Pre-login flow detected, calling resetAuthState to cancel IDV flow');
        await rdnaService.resetAuthState();
        console.log('IDVSelfieConfirmation - ResetAuthState successful');
      } else {
        // Post-login flow: Navigate to ActivatedCustomerKYC screen (button will be auto-enabled)
        console.log('IDVSelfieConfirmation - Post-login flow detected, navigating to ActivatedCustomerKYC screen');
        ...
        console.log('IDVSelfieConfirmation - Navigated to ActivatedCustomerKYC, button will be auto-enabled');
      }
    } catch (error) {
      console.error('IDVSelfieConfirmation - Close button error:', JSON.stringify(error, null, 2));
