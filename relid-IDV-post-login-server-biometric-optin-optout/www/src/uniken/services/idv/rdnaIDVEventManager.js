...
class RdnaIDVEventManager {
  constructor() {
    ...
    // Biometric Opt-In/Opt-Out Event Handlers
    this.optInCapturedFrameConfirmationHandler = null;
    this.biometricOptInStatusHandler = null;
    this.biometricOptOutStatusHandler = null;
  ...
  registerEventListeners() {
    ...
    // Biometric Opt-In Captured Frame Confirmation Event
    document.addEventListener(
      'onIDVOptInCapturedFrameConfirmation',
      this.onOptInCapturedFrameConfirmation.bind(this),
      false
    );

    // Biometric Opt-In Status Event
    document.addEventListener(
      'onIDVBiometricOptInStatus',
      this.onBiometricOptInStatus.bind(this),
      false
    );

    // Biometric Opt-Out Status Event
    document.addEventListener(
      'onIDVBiometricOptOutStatus',
      this.onBiometricOptOutStatus.bind(this),
      false
    );
  ...
  /**
   * Handles biometric opt-in captured frame confirmation event
   *
   * This event is triggered during the biometric opt-in flow after the SDK captures
   * a live selfie frame with liveness detection. The event provides the captured
   * image as base64 data for user review and confirmation.
   *
   * The event data includes:
   * - capturedImage: Base64-encoded image data of the captured selfie frame
   *
   * Application should:
   * - Display the captured image in a modal for user review
   * - Provide three action buttons: Approve, Recapture, Cancel
   * - Call setIDVBiometricOptInConfirmation() with user's selected action:
   *   - 0 (RDNA_IDV_APPROVED): User approves image, proceed with template creation
   *   - 1 (RDNA_IDV_RECAPTURE): User requests recapture (modal stays open for new image)
   *   - 2 (RDNA_IDV_CANCEL): User cancels opt-in flow
   * - Keep modal open if recapture selected (prevents UI flickering)
   * - Dismiss modal only after receiving onIDVBiometricOptInStatus event
   *
   * Flow Context:
   * - Part of Workflow 10 (Biometric Opt-In)
   * - Challenge Mode 6
   * - Follows user authentication (LDA or password)
   * - Precedes template creation and server storage
   *
   * @param event Event object containing captured frame data from native SDK
   */
  onOptInCapturedFrameConfirmation(event) {
    console.log('RdnaIDVEventManager - Opt-in captured frame confirmation event received');

    try {
      const response = event.response || event.detail || event;
      console.log('RdnaIDVEventManager - onOptInCapturedFrameConfirmation response received');

      // Response is direct base64 string (not JSON-wrapped)
      const data = {
        capturedImage: typeof response === 'string' ? response : response.capturedImage || response.response
      };

      console.log('RdnaIDVEventManager - Captured image data length:', data.capturedImage?.length || 0);

      if (this.optInCapturedFrameConfirmationHandler) {
        this.optInCapturedFrameConfirmationHandler(data);
      }
    } catch (error) {
      console.error('RdnaIDVEventManager - Failed to parse opt-in captured frame confirmation:', error);
    }
  }

  /**
   * Handles biometric opt-in status event
   *
   * This event is triggered after the biometric opt-in process completes, indicating
   * whether the template was successfully created and stored on the server. This is
   * the final event in the opt-in workflow.
   *
   * The event data includes:
   * - userID: User identifier
   * - challengeMode: Challenge operation mode (6 for opt-in)
   * - attemptsLeft: Remaining attempts (if applicable)
   * - idvResponse: JSON string containing opt-in result
   *   - status_code: Result code (100 = success, others = failure/error)
   *   - status_message: Human-readable status message
   *   - idv_audit_info: Audit information including transaction ID
   *   - orchestration_use_case: Use case identifier
   * - status: Status object with statusCode
   * - error: Error information with codes and message
   *
   * Application should:
   * - Check error.longErrorCode for API errors
   * - Parse idvResponse JSON string for detailed status
   * - Check status_code: 100 = template created successfully
   * - Display success/failure modal to user
   * - Update UI to reflect new biometric enrollment status
   * - Navigate appropriately (e.g., back to dashboard)
   *
   * Status Codes:
   * - 100: Template successfully created and stored
   * - Other codes: Template creation failed
   *
   * Flow Context:
   * - Final event in Workflow 10 (Biometric Opt-In)
   * - Challenge Mode 6
   * - Follows captured frame approval
   * - After this event, template is available for authentication
   *
   * @param event Event object containing opt-in status from native SDK
   */
  onBiometricOptInStatus(event) {
    console.log('RdnaIDVEventManager - Biometric opt-in status event received');

    try {
      const response = event.response || event.detail || event;
      console.log('RdnaIDVEventManager - onBiometricOptInStatus response:', JSON.stringify(response, null, 2));
      ...
      if (data.idvResponse) {
        try {
          const idvData = JSON.parse(data.idvResponse);
          console.log('RdnaIDVEventManager - Opt-In Status Code:', idvData.status_code);
          console.log('RdnaIDVEventManager - Opt-In Status Message:', idvData.status_message);
          console.log('RdnaIDVEventManager - Transaction ID:', idvData.idv_audit_info?.idv_audit_transaction_id);
          console.log('RdnaIDVEventManager - Orchestration Use Case:', idvData.orchestration_use_case);
        } catch (parseError) {
          console.error('RdnaIDVEventManager - Failed to parse idvResponse JSON:', parseError);
      ...
      if (this.biometricOptInStatusHandler) {
        this.biometricOptInStatusHandler(data);
      }
    } catch (error) {
      console.error('RdnaIDVEventManager - Failed to parse biometric opt-in status:', error);
    }
  }

  /**
   * Handles biometric opt-out status event
   *
   * This event is triggered after the biometric opt-out process completes, indicating
   * whether the template was successfully deleted from the server. This is the final
   * event in the opt-out workflow.
   *
   * The event data includes:
   * - userID: User identifier
   * - challengeMode: Challenge operation mode (7 for opt-out)
   * - attemptsLeft: Remaining attempts (if applicable)
   * - idvResponse: JSON string containing opt-out result
   *   - status_code: Result code (100 = success, others = failure/error)
   *   - status_message: Human-readable status message
   * - status: Status object with statusCode
   * - error: Error information with codes and message
   ...
   * - Check error.longErrorCode for API errors
   * - Parse idvResponse JSON string for detailed status
   * - Check status_code: 100 = template deleted successfully
   * - Display success/failure modal to user
   * - Update UI to reflect biometric template removal
   * - Navigate appropriately (e.g., back to dashboard)
   *
   * Status Codes:
   * - 100: Template successfully deleted
   * - Other codes: Template deletion failed
   *
   * Flow Context:
   * - Final event in Workflow 11 (Biometric Opt-Out)
   * - Challenge Mode 7
   * - Follows user authentication (LDA or password)
   * - No selfie capture in this flow (unlike opt-in)
   * - After this event, template is removed from server
   *
   * @param event Event object containing opt-out status from native SDK
   */
  onBiometricOptOutStatus(event) {
    console.log('RdnaIDVEventManager - Biometric opt-out status event received');

    try {
      const response = event.response || event.detail || event;
      console.log('RdnaIDVEventManager - onBiometricOptOutStatus response:', JSON.stringify(response, null, 2));
      ...
      if (data.idvResponse) {
        try {
          const idvData = JSON.parse(data.idvResponse);
          console.log('RdnaIDVEventManager - Opt-Out Status Code:', idvData.status_code);
          console.log('RdnaIDVEventManager - Opt-Out Status Message:', idvData.status_message);
        } catch (parseError) {
          console.error('RdnaIDVEventManager - Failed to parse idvResponse JSON:', parseError);
        }
      }

      if (this.biometricOptOutStatusHandler) {
        this.biometricOptOutStatusHandler(data);
      }
    } catch (error) {
      console.error('RdnaIDVEventManager - Failed to parse biometric opt-out status:', error);
    }
  }
  ...
  /**
   * Biometric Opt-In/Opt-Out Event Handler Setters
   */

  setOptInCapturedFrameConfirmationHandler(callback) {
    this.optInCapturedFrameConfirmationHandler = callback;
  }

  setBiometricOptInStatusHandler(callback) {
    this.biometricOptInStatusHandler = callback;
  }

  setBiometricOptOutStatusHandler(callback) {
    this.biometricOptOutStatusHandler = callback;
  }
  ...
  cleanup() {
    ...
    document.removeEventListener('onIDVOptInCapturedFrameConfirmation', this.onOptInCapturedFrameConfirmation.bind(this), false);
    document.removeEventListener('onIDVBiometricOptInStatus', this.onBiometricOptInStatus.bind(this), false);
    document.removeEventListener('onIDVBiometricOptOutStatus', this.onBiometricOptOutStatus.bind(this), false);
    ...
    // Clear biometric opt-in/opt-out handlers
    this.optInCapturedFrameConfirmationHandler = null;
    this.biometricOptInStatusHandler = null;
    this.biometricOptOutStatusHandler = null;
