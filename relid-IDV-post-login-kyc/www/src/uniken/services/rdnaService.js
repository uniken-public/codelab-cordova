...
class RdnaService {
  ...
  /**
   * Authenticates user and signs data with cryptographic signature
   *
   * Initiates the data signing process with specified authentication parameters.
   * This method has both sync callback AND async event (onAuthenticateUserAndSignData).
   * May also trigger getPassword for step-up authentication during the flow.
   *
   * @see https://developer.uniken.com/docs/data-signing
   *
   * Workflow:
   * 1. User submits data signing request from DataSigningInputScreen
   * 2. Screen calls authenticateUserAndSignData(payload, authLevel, authenticatorType, reason)
   * 3. SDK may trigger getPassword event for step-up authentication
   * 4. After authentication (if required), SDK processes signing
   * 5. SDK triggers onAuthenticateUserAndSignData event with signed data results
   * 6. Event contains signature, signature ID, and metadata
   *
   * Authentication Levels:
   * - 0: NONE - No authentication required
   * - 1: RDNA_AUTH_LEVEL_1 - Basic authentication
   * - 2: RDNA_AUTH_LEVEL_2 - Standard authentication
   * - 3: RDNA_AUTH_LEVEL_3 - Enhanced authentication
   * - 4: RDNA_AUTH_LEVEL_4 - Maximum security (recommended)
   *
   * Authenticator Types:
   * - 0: NONE - No authenticator
   * - 1: RDNA_IDV_SERVER_BIOMETRIC - Server-based biometric
   * - 2: RDNA_AUTH_PASS - Password authentication
   * - 3: RDNA_AUTH_LDA - Local device authentication (biometric)
   *
   * Response Structure (Sync):
   * {
   *   error: { longErrorCode, shortErrorCode, errorString }
   * }
   *
   * Event Structure (Async - onAuthenticateUserAndSignData):
   * {
   ...
   *   authenticationType: number,       // Authenticator type used
   *   status: {
   *     statusCode: number,             // Status code (100 = success)
   ...
   *
   * Response Validation Logic:
   * 1. Check sync response error.longErrorCode: 0 = success, > 0 = error
   * 2. Wait for onAuthenticateUserAndSignData event for signing results
   * 3. Handle intermediate getPassword events for step-up authentication
   * 4. Check event status.statusCode: 100 = success
   *
   * @param {string} payload - Data payload to sign (max 500 characters)
   * @param {number} authLevel - Authentication level (0-4)
   * @param {number} authenticatorType - Authenticator type (0-3)
    ...
    return new Promise((resolve, reject) => {
      ...
      com.uniken.rdnaplugin.RdnaClient.authenticateUserAndSignData(
        (response) => {
          ...
          const result = JSON.parse(response);
          console.log('RdnaService - authenticateUserAndSignData sync response:', JSON.stringify({
            longErrorCode: result.error?.longErrorCode,
            shortErrorCode: result.error?.shortErrorCode,
            errorString: result.error?.errorString
          }, null, 2));
          ...
          // Always resolve with result (let caller check error.longErrorCode)
          // Async event (onAuthenticateUserAndSignData) will follow after authentication
          console.log('RdnaService - AuthenticateUserAndSignData sync response, waiting for authentication and signing');
          resolve(result);
        },
        (error) => {
          console.error('RdnaService - authenticateUserAndSignData error callback:', error);
          const result = JSON.parse(error);
          console.error('RdnaService - authenticateUserAndSignData sync error:', JSON.stringify({
            longErrorCode: result.error?.longErrorCode,
            shortErrorCode: result.error?.shortErrorCode,
            errorString: result.error?.errorString
          }, null, 2));
        ...
        [payload, authLevel, authenticatorType, reason] // [PAYLOAD, AUTH_LEVEL, AUTHENTICATOR_TYPE, REASON]
      );
    });
  }
   ...
   *
   * Use Cases:
   * - After successful data signing completion
   * - When user cancels data signing flow
   * - When navigating away from data signing screens
   * - On error during data signing process
   *
   * @see https://developer.uniken.com/docs/data-signing
   *
   * Response Structure (Sync only - no async event):
   * {
   *   error: { longErrorCode, shortErrorCode, errorString }
   * }
   *
    ...
    return new Promise((resolve, reject) => {
      ...
      com.uniken.rdnaplugin.RdnaClient.resetAuthenticateUserAndSignDataState(
        (response) => {
          ...
          const result = JSON.parse(response);
          console.log('RdnaService - resetAuthenticateUserAndSignDataState sync response:', JSON.stringify({
            longErrorCode: result.error?.longErrorCode,
            shortErrorCode: result.error?.shortErrorCode,
            errorString: result.error?.errorString
          }, null, 2));
          ...
          if (result.error && result.error.longErrorCode === 0) {
            console.log('RdnaService - Data signing state reset successfully');
            resolve(result);
          } else {
            console.error('RdnaService - Failed to reset data signing state:', JSON.stringify(result, null, 2));
            reject(result);
          }
        },
        (error) => {
          console.error('RdnaService - resetAuthenticateUserAndSignDataState error callback:', error);
          const result = JSON.parse(error);
          console.error('RdnaService - resetAuthenticateUserAndSignDataState sync error:', JSON.stringify({
            longErrorCode: result.error?.longErrorCode,
            shortErrorCode: result.error?.shortErrorCode,
            errorString: result.error?.errorString
          }, null, 2));
  ...
  /**
   * Gets registered device details for device management
   *
   * Retrieves the list of all registered devices for the authenticated user.
   * This method follows the sync+async pattern: sync callback returns immediately,
   * then triggers an onGetRegistredDeviceDetails event with device list data.
   *
   * @see https://developer.uniken.com/docs/device-management
   *
   * Workflow:
   * 1. User navigates to Device Management screen from drawer menu
   * 2. Screen calls getRegisteredDeviceDetails(userId)
   * 3. Sync callback returns immediately with error structure
   * 4. SDK triggers onGetRegistredDeviceDetails event with device list
   * 5. Event contains array of devices with metadata (UUID, name, status, timestamps)
   * 6. Screen renders device list with current device highlighting
   *
   * Device Object Structure:
   * {
   *   devUUID: string,              // Unique device identifier
   *   devName: string,              // User-assigned device name
   *   status: string,               // "ACTIVE" or "INACTIVE"
   *   lastAccessedTs: string,       // Last access timestamp (ISO format)
   *   lastAccessedTsEpoch: number,  // Last access timestamp (epoch ms)
   *   createdTs: string,            // Creation timestamp (ISO format)
   *   createdTsEpoch: number,       // Creation timestamp (epoch ms)
   *   appUuid: string,              // Application UUID
   ...
   * }
   *
   * Event Structure (Async - onGetRegistredDeviceDetails):
   * {
   *   errCode: number,
   *   error: { longErrorCode, shortErrorCode, errorString },
   *   eMethId: number,
   ...
   *     response: {
   *       ResponseData: {
   *         device: [Device[]],                        // Array of device objects
   *         deviceManagementCoolingPeriodEndTimestamp: number  // Cooling period end time (epoch ms)
   *       },
   *       StatusMsg: string,
   *       StatusCode: number,  // 100 = success, 146 = cooling period active
   *       CredOpMode: number
   *     }
   *   }
   * }
   ...
   * 3. Check StatusCode: 100 = success, 146 = cooling period (operations disabled)
   * 4. Parse device array from ResponseData.device
   * 5. Check deviceManagementCoolingPeriodEndTimestamp for cooling period end time
   *
  ...
  /**
   * Sets the SDK language dynamically after initialization
   *
   * This API allows changing the SDK's language configuration without re-initializing.
   * Useful for apps that support runtime language switching.
   *
   * Flow:
   * 1. User selects new language in UI
   * 2. App calls setSDKLanguage(localeCode, languageDirection)
   * 3. SDK validates and applies the language change
   * 4. SDK triggers onSetLanguageResponse event with updated configuration
   * 5. SDKEventProvider.handleSetLanguageResponse() updates LanguageManager
   *
   * @param {string} localeCode - Full locale code (e.g., 'en-US', 'hi-IN', 'ar-SA')
   * @param {number} languageDirection - 0 = LTR, 1 = RTL
   * @returns {Promise<Object>} Promise that resolves with sync response
   *
   * @example
   * // Change to Hindi (LTR)
   * await rdnaService.setSDKLanguage('hi-IN', 0);
   *
   * @example
   * // Change to Arabic (RTL)
   * await rdnaService.setSDKLanguage('ar-SA', 1);
   */
  async setSDKLanguage(localeCode, languageDirection) {
    return new Promise((resolve, reject) => {
      ...
      com.uniken.rdnaplugin.RdnaClient.setSDKLanguage(
        ...
        [localeCode, languageDirection] // [LOCALE_CODE, LANGUAGE_DIRECTION]
      );
    });
  }
}
