...
class RdnaEventManager {
  constructor() {
    ...
    this.getRegisteredDeviceDetailsHandler = null;
    this.updateDeviceDetailsHandler = null;

    // Language Management event handlers
  ...
  registerEventListeners() {
    ...
    const getRegisteredDeviceDetailsListener = this.onGetRegistredDeviceDetails.bind(this);
    const updateDeviceDetailsListener = this.onUpdateDeviceDetails.bind(this);
    ...
    // Language Management event registrations
    ...
    this.listeners.push(
      ...
      { name: 'onGetRegistredDeviceDetails', handler: getRegisteredDeviceDetailsListener },
      { name: 'onUpdateDeviceDetails', handler: updateDeviceDetailsListener },
      { name: 'onSetLanguageResponse', handler: setLanguageResponseListener }
    );

    console.log('RdnaEventManager - Native event listeners registered (including MTD, MFA, Session, Device Activation, Notifications, Password Update, LDA Toggling, Data Signing, Device Management, and Language Management)');
  }
   ...
   * This event is triggered after calling setSDKLanguage() API.
   * Contains updated language configuration and supported languages from SDK.
   *
   * Response Format:
   * {
   *   localeCode: "hi-IN",
   ...
   *   ],
   *   status: { statusCode: 100, statusMessage: "Success" },
   *   error: { longErrorCode: 0, shortErrorCode: 0, errorString: "" }
   * }
   *
   * @param {RDNAJsonResponse} event - Event from native SDK containing language change response
   */
  onSetLanguageResponse(event) {
    ...
    try {
      ...
      console.log("RdnaEventManager - Language change response:", JSON.stringify({
  ...
  /**
   * Sets handler for set language response events
   * @param {Function} callback - Callback function receiving language change response data
   */
  ...
  cleanup() {
    ...
    // Clear Device Management event handlers
    this.getRegisteredDeviceDetailsHandler = null;
    this.updateDeviceDetailsHandler = null;

    // Clear Language Management event handlers
    this.setLanguageResponseHandler = null;
